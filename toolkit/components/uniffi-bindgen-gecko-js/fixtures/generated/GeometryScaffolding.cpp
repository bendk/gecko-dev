...
namespace geometry_c382_gradient {
...
Result Invoke(Args& aArgs) {
    Result result = {};result.mReturnValue =::geometry_c382_gradient(
...
// For each Rust scaffolding function, define types and functions for calling it
namespace geometry_c382_intersection {
using namespace mozilla::dom;
// Arguments to pass to the scaffolding function
struct Args {
    // RustBuffer arguments are stored as OwnedRustBuffer instances.  That class takes care of
...
// For async calls this should be called in the worker thread
Result Invoke(Args& aArgs) {
    Result result = {};result.mReturnValue =::geometry_c382_intersection(
         // Call intoRustBuffer() to get a `RustBuffer` from an OwnedRustBuffer.  This transfers ownership to Rust,
         // OwnedRustBuffer will no longer try to free it
...
already_AddRefed<Promise> GeometryScaffolding::GeometryC382Gradient(const GlobalObject& aUniFFIGlobal,const ArrayBuffer& ln,
 ErrorResult& aUniFFIError) {
    ...
    auto uniFFIArgs = geometry_c382_gradient::PrepareArgs(ln, aUniFFIError);
    if (aUniFFIError.Failed()) {
        return nullptr;
    }

    // Create a second promise that gets resolved by a background task that calls the scaffolding function
    using UniFFITaskPromise = MozPromise<geometry_c382_gradient::Result, nsresult, true>;
    ...
    nsresult uniFFIDispatchResult = NS_DispatchBackgroundTask(
            NS_NewRunnableFunction(
                "GeometryScaffolding::GeometryC382Gradient",
                [args = std::move(uniFFIArgs), uniFFITaskPromise]() mutable {
                auto result = geometry_c382_gradient::Invoke(args);
    ...
    uniFFITaskPromise->Then(GetCurrentSerialEventTarget(), __func__,
            ...
            if (!aResult.IsResolve()) {
                uniFFIReturnPromise->MaybeRejectWithUnknownError("GeometryScaffolding::GeometryC382Gradient task dispatch failed");
                return;
            }

            AutoEntryScript aes(uniFFIXPCOMGlobal, "GeometryScaffolding::GeometryC382Gradient call resolve");
            RootedDictionary<UniFFIRustCallResult> returnValue(aes.cx());

            geometry_c382_gradient::ReturnResult(aes.cx(), aResult.ResolveValue(), returnValue);
...
using namespace uniffi::geometry;
already_AddRefed<Promise> GeometryScaffolding::GeometryC382Intersection(const GlobalObject& aUniFFIGlobal,const ArrayBuffer& ln1, const ArrayBuffer& ln2,
 ErrorResult& aUniFFIError) {
    // Note: Prefix our params and local variables with "uniffi" to avoid name
    // conflicts with the scaffolding function args
    ...
    // Prepare arguments to pass to Rust
    auto uniFFIArgs = geometry_c382_intersection::PrepareArgs(ln1, ln2, aUniFFIError);
    if (aUniFFIError.Failed()) {
        return nullptr;
    }

    // Create a second promise that gets resolved by a background task that calls the scaffolding function
    using UniFFITaskPromise = MozPromise<geometry_c382_intersection::Result, nsresult, true>;
    RefPtr uniFFITaskPromise = new UniFFITaskPromise::Private(__func__);
    nsresult uniFFIDispatchResult = NS_DispatchBackgroundTask(
            NS_NewRunnableFunction(
                "GeometryScaffolding::GeometryC382Intersection",
                [args = std::move(uniFFIArgs), uniFFITaskPromise]() mutable {
                auto result = geometry_c382_intersection::Invoke(args);
                uniFFITaskPromise->Resolve(std::move(result), __func__);
            }),
            NS_DISPATCH_EVENT_MAY_BLOCK);
    if (NS_FAILED(uniFFIDispatchResult)) {
    ...
    uniFFITaskPromise->Then(GetCurrentSerialEventTarget(), __func__,
            [uniFFIXPCOMGlobal, uniFFIReturnPromise](UniFFITaskPromise::ResolveOrRejectValue&& aResult) {
            if (!aResult.IsResolve()) {
                uniFFIReturnPromise->MaybeRejectWithUnknownError("GeometryScaffolding::GeometryC382Intersection task dispatch failed");
                return;
            }

            AutoEntryScript aes(uniFFIXPCOMGlobal, "GeometryScaffolding::GeometryC382Intersection call resolve");
            RootedDictionary<UniFFIRustCallResult> returnValue(aes.cx());

            geometry_c382_intersection::ReturnResult(aes.cx(), aResult.ResolveValue(), returnValue);
            uniFFIReturnPromise->MaybeResolve(returnValue);
            }
    );
