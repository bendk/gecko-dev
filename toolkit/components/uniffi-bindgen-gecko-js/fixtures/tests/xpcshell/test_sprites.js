...
const Sprites = ChromeUtils.import("resource://gre/modules/components-utils/Sprites.jsm")

add_task(async function() {
    Assert.ok(Sprites.Sprite);

    sempty = await Sprites.Sprite.init(null)
    Assert.deepEqual(await sempty.getPosition(), new Sprites.Point(0, 0))

    s = await Sprites.Sprite.init(new Sprites.Point(0, 1))
    ...
    srel = await Sprites.Sprite.newRelativeTo(new Sprites.Point(0, 1), new Sprites.Vector(1, 1.5))
