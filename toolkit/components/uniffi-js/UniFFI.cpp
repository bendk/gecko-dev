...
already_AddRefed<Promise> UniFFI::CallAsync(
    const GlobalObject& aGlobal, uint64_t aId,
    const Sequence<ScaffoldingType>& aArgs, ErrorResult& aError) {
  Maybe<already_AddRefed<Promise>> firstTry =
      UniFFICallAsync(aGlobal, aId, aArgs, aError);
  if (firstTry.isSome()) {
    return firstTry.extract();
  }
#ifdef MOZ_UNIFFI_FIXTURES
  Maybe<already_AddRefed<Promise>> secondTry =
      UniFFIFixturesCallAsync(aGlobal, aId, aArgs, aError);
  if (secondTry.isSome()) {
...
void UniFFI::CallSync(
    const GlobalObject& aGlobal, uint64_t aId,
    const Sequence<ScaffoldingType>& aArgs,
    RootedDictionary<UniFFIScaffoldingCallResult>& aReturnValue,
    ErrorResult& aError) {
  if (UniFFICallSync(aGlobal, aId, aArgs, aReturnValue, aError)) {
    return;
  }
#ifdef MOZ_UNIFFI_FIXTURES
  if (UniFFIFixturesCallSync(aGlobal, aId, aArgs, aReturnValue, aError)) {
    return;
  }
#endif
...
already_AddRefed<UniFFIPointer> UniFFI::ReadPointer(
    const GlobalObject& aGlobal, uint64_t aId, const ArrayBuffer& aArrayBuff,
    long aPosition, ErrorResult& aError) {
  Maybe<already_AddRefed<UniFFIPointer>> firstTry =
      UniFFIReadPointer(aGlobal, aId, aArrayBuff, aPosition, aError);
  if (firstTry.isSome()) {
    return firstTry.extract();
  }
#ifdef MOZ_UNIFFI_FIXTURES
  Maybe<already_AddRefed<UniFFIPointer>> secondTry =
      UniFFIFixturesReadPointer(aGlobal, aId, aArrayBuff, aPosition, aError);
  if (secondTry.isSome()) {
    return secondTry.extract();
  }
#endif
...
void UniFFI::WritePointer(const GlobalObject& aGlobal, uint64_t aId,
                          const UniFFIPointer& aPtr,
                          const ArrayBuffer& aArrayBuff, long aPosition,
                          ErrorResult& aError) {
  if (UniFFIWritePointer(aGlobal, aId, aPtr, aArrayBuff, aPosition, aError)) {
    return;
  }
#ifdef MOZ_UNIFFI_FIXTURES
  if (UniFFIFixturesWritePointer(aGlobal, aId, aPtr, aArrayBuff, aPosition,
                                 aError)) {
    return;
  }
#endif
