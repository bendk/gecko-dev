...
class ScaffoldingConverter {
 public:
  ...
  // Convert an intermedate type to a Rust type
  //
...
class ScaffoldingConverter<RustBuffer> {
 public:
  ...
  static mozilla::Result<OwnedRustBuffer, nsCString> FromJs(
    ...
    const ArrayBuffer& arrayBuf = aValue.GetAsArrayBuffer();
...
class ScaffoldingConverter<PointerType, ScaffoldingConverterTagObject> {
 public:
  using RustType = void*;
